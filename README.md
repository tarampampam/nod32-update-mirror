# !!! ДАННАЯ ВЕТВЬ БОЛЕЕ НЕ ПОДДЕРЖИВАЕТСЯ !!!
...
#### <i class="icon-file"></i>Особенности
 ...
 - В комплекте идет заготовка для веб-интерфейса зеркала обновления (директория `./webface`, [демо](https://googledrive.com/host/0B8jho7c8kaRhWVBoeTJzVnowUW8/)). Поддерживаются веб-серверы: **nginx** (под **Apache 1.x** и  **Apache 2.x** работать, разумеется, будет, но раздавать им статику крайне не правильно; настройки этих веб-серверов не обновляются в данном репозитории).
